...
public class NetworkChannelHelper : INetworkChannelHelper
{
    ...
    private EventComponent Event => GameEntry.GetComponent<EventComponent>();
    ...
    public void Initialize(INetworkChannel networkChannel)
    {
        ...
        for (int i = 0; i < types.Length; i++)
        {
            ...
            if (types[i].BaseType.IsGenericType && types[i].BaseType.GetGenericTypeDefinition() == packetBaseType)
            {
        ...
        Event.Subscribe(UnityGameFramework.Runtime.NetworkMissHeartBeatEventArgs.EventId,
    ...
    {
        Event.Unsubscribe(UnityGameFramework.Runtime.NetworkConnectedEventArgs.EventId,
            OnNetworkConnected);
        Event.Unsubscribe(UnityGameFramework.Runtime.NetworkClosedEventArgs.EventId, OnNetworkClosed);
        Event.Unsubscribe(UnityGameFramework.Runtime.NetworkMissHeartBeatEventArgs.EventId,
            OnNetworkMissHeartBeat);
        Event.Unsubscribe(UnityGameFramework.Runtime.NetworkErrorEventArgs.EventId, OnNetworkError);
        Event.Unsubscribe(UnityGameFramework.Runtime.NetworkCustomErrorEventArgs.EventId,
            OnNetworkCustomError);
    ...
    public bool Serialize<T>(T packet, Stream destination) where T : Packet
    {
        ...
        // 将消息头写入m_CachedStream中
        CSPacketHeader packetHeader = ReferencePool.Acquire<CSPacketHeader>();
        // 在这里要记录下消息体的长度，因为Release后数据会被清除
        int packetLength = packetImpl.GetMsgSize();
        packetHeader.Id = packet.Id;
        packetHeader.PacketLength = packetLength;
        packetHeader.Serialize(m_CachedStream);
        ReferencePool.Release(packetHeader);

        // 将消息体写入m_CachedStream中
        packetImpl.Serialize(m_CachedStream);
        ReferencePool.Release(packet);

        // 将m_CachedStream写入destination中
        int sendLength = PacketHeaderLength + packetLength;
        destination.SetLength(sendLength);
        destination.Position = 0L;
        m_CachedStream.SetLength(sendLength);
